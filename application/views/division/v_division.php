<main id="main">
	<!-- ======= Breadcrumbs ======= -->
	<section id="breadcrumbs" class="breadcrumbs">
		<div class="container">
			<div class="d-flex justify-content-between align-items-center">
				<h2>Division</h2>
			</div>
		</div>
	</section><!-- End Breadcrumbs -->

	<!-- ======= Our Team Section ======= -->
	<section id="team" class="team section-bg">
		<div class="container">

			<div class="section-title mt-5" data-aos="fade-up">
				<h2>Board of<strong> Directors</strong></h2>
			</div>

			<!-- Executive Director -->
			<div class="row d-flex justify-content-center">
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Valencia_Wijaya.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/albimarangga/ "><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/albima-rangga-setiyawan-501b531b5/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Valencia Wijaya</h4>
							<span>Executive Director</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
			</div>

			<!-- Staff first line -->
			<div class="row d-flex justify-content-center">
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="300">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Jessica.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/fei.slsbll/"><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/feira-salsabila-b5aa691b3/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Jessica Evangeli Santoso</h4>
							<span>General Secretary</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="400">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Alexius.png" class="img-fluid" alt="">
							<!-- <div class="social">

								<a href="https://www.instagram.com/hanuritaaa/"><i class="icofont-instagram"></i></a>
								<a href=""><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Alexius Krisnantyo Wijaya</h4>
							<span>Vice Secretary</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="500">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Togito.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/matthewrianto/"><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/matthew-aprianto-81430a1b2/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Togito Yohanes Parulian</h4>
							<span>Treasurer</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="600">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Felina.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/jesicajillv/"><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/jesica-jill-valentine-bb8494134/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Felina</h4>
							<span>Vice Treasurer</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
			</div>

			<!-- Staff Second Line -->
			<div class="row d-flex justify-content-center">
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="700">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Meidita.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/james.xaverius/"><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/james-xaverius/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Meidita Rahmawati</h4>
							<span>Director of Research</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="800">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Gabriella.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/albimarangga/ "><i class="icofont-instagram"></i></a>
								<a href="https://www.linkedin.com/in/albima-rangga-setiyawan-501b531b5/"><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Gabriella Jasmine Mulyadi</h4>
							<span>Director of Media and Public Relation</span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
				<div class="col-lg-3 col-md-6 d-flex align-items-stretch">
					<div class="member" data-aos="fade-up" data-aos-delay="900">
						<div class="member-img">
							<img src="assets/img/abls/BOE/Patricia.png" class="img-fluid" alt="">
							<!-- <div class="social">
								<a href="https://www.instagram.com/gandhiwicky/"><i class="icofont-instagram"></i></a>
								<a href=""><i class="icofont-linkedin"></i></a>
							</div> -->
						</div>
						<div class="member-info">
							<h4>Patricia Ingrid Dewani</h4>
							<span>Director of Organizations</Span>
							<span>of</span>
							<span>Atma Jaya BLS</span>
							<span>Board of 2022</span>
						</div>
					</div>
				</div>
			</div>

			<hr color="#A33020" width="100%" size="75%">
			<div class="row d-flex justify-content-center" data-aos="fade-up">
				<a href="<?= base_url('research') ?>" class="btn btn-gold">Department of Research</a>
				<a href="<?= base_url('media') ?>" class="btn btn-gold">Media & Public Relations</a>
				<a href="<?= base_url('organizations') ?>" class="btn btn-gold">Departement of Organizations</a>
			</div>
		</div>
	</section><!-- End Our Team Section -->
</main><!-- End #main -->
